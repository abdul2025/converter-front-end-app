...
@Component({
  ...
  styleUrls: ['./excel-to-pdf.component.scss']
})
export class ExcelToPdfComponent implements OnInit {
  ...
  private handleFileSelection(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (this.allowedExcelTypes.includes(file.type)) {
        this.selected = file;
      }
    }
  }
  ...
  onDropFile(event: DragEvent): void {
    ...
    if (event.dataTransfer?.files.length) {
      ...
      if (this.allowedExcelTypes.includes(file.type)) {
        ...
        this.errorHandlerService.showErrorMessage('Please drop a Excel or csv file.')
      }
    }
  }
  ...
  convertAction(): void {
    this.apiService.convertToPdf(this.selected!, 'excel')
    .subscribe({
      next: (response) => {
